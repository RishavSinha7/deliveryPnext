...
import { MapPin, Clock, DollarSign, Navigation, Package, Phone, RefreshCw } from 'lucide-react';
...
export const ActiveTripsTab = ({ trips }: ActiveTripsTabProps): JSX.Element => {
  ...
  const handleStatusUpdate = async (bookingId: string, newStatus: string) => {
    ...
    try {
      ...
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5000/api'}/bookings/${bookingId}/status`, {
        method: 'PUT',
        headers: {
